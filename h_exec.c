...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPreviousInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if(!lpCmdLine || !*lpCmdLine) {			// Is there a command line?
		...
		if(bSearchPath) {
			...
			bRet = (intptr_t)ShellExecute(GetDesktopWindow(), NULL, szPath, szParam, NULL, SW_SHOWNORMAL);

		} else {

			bRet = (intptr_t)ShellExecute(GetDesktopWindow(), NULL, szRun, szParam, NULL, SW_SHOWNORMAL);
		}
		...
		if(!strncmp(lpCmdLine, "reg:", 4)) {
			...
			strcat(szExec, szPgm);
			if(ExistFile(szExec))
				bRet = (intptr_t)ShellExecute(GetDesktopWindow(), "open", szExec, NULL, NULL, SW_SHOWNORMAL);
			else
				bRet = (intptr_t)ShellExecute(GetDesktopWindow(), "open", szAlt, NULL, NULL, SW_SHOWNORMAL);
		} else

			// Must be a command line, then.

			bRet = (intptr_t)ShellExecute(GetDesktopWindow(), "open", lpCmdLine, NULL, NULL, SW_SHOWNORMAL);
	}
